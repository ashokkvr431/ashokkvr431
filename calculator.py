...
result_label = Label(root, text='', bg='lightblue', fg='black')
result_label.grid(row=0, column=0, columnspan=10, pady=25, sticky='e')
result_label.config(font=('arial', 40, 'normal'))
...
button_add = Button(root, text='+', bg='yellow', fg='black', width=5, height=2, command=lambda: get_operator('+'))
...
button_sub = Button(root, text='-', bg='yellow', fg='black', width=5, height=2, command=lambda: get_operator('-'))
...
button_multi = Button(root, text='*', bg='yellow', fg='black', width=5, height=2, command=lambda: get_operator('*'))
button_multi.grid(row=3, column=3)
button_multi.config(font=('calibri', 20))

button0 = Button(root, text='AC', bg='lightblue', fg='black', width=5, height=2, command=lambda: clear())
button0.grid(row=4, column=0)
button0.config(font=('calibri', 20))

button_clear = Button(root, text='0', bg='grey', fg='black', width=5, height=2, command=lambda: get_digit(0))
button_clear.grid(row=4, column=1)
button_clear.config(font=('calibri', 20))

button_eql = Button(root, text='=', bg='red', fg='black', width=5, height=2, command=get_result)
button_eql.grid(row=4, column=2)
button_eql.config(font=('calibri', 20))

button_div = Button(root, text='/', bg='yellow', fg='black', width=5, height=2, command=lambda: get_operator('/'))
